...
// loadData is most main func because issme ham sab type like filter,sort,search,default type ke api get kar rahe hai.
  const loadData = async (start, end, increase,optType=null,FilterOrSearch) => {
    ...
    switch (optType){
      ...
      case "search":
        ...
        case "sort":
          ...
          return await axios
      .get(`http://localhost:5000/users?_sort=${FilterOrSearch}&_order=asc&_start=${start}&_end=${end}`)
      .then((response) => {
        setData(response.data);
        setCurrentPage(currentPage+increase)
      })
      ...
      case "filter":
        setOperation(optType)
          ...
          return await axios
      .get(`http://localhost:5000/users?status=${FilterOrSearch}&_order=asc&_start=${start}&_end=${end}`)
      .then((response) => {
        setData(response.data);
        setCurrentPage(currentPage+increase)
      })
  ...
  const renderPagination = () => {;
    ...
    if (currentPage === 0) {
        ...
        <div className={styles.paginationSection}>
          <button
            onClick={() => {
              ...
              loadData((currentPage - 1) * 4, currentPage * 4, -1,operation,sortFilterValue);
            }}
          >
            Prev
          </button>
          ...
          <button
            onClick={() => {
              ...
              loadData((currentPage + 1) * 4, (currentPage + 2) * 4, 1,operation,sortFilterValue);
      ...
      return (
        <div className={styles.paginationSection}>
          <button onClick={() => loadData((currentPage-1)*4, (currentPage*4), -1,operation,sortFilterValue)}>Prev</button>
          <p className={styles.NumberOfPage}>{currentPage + 1}</p>
        </div>
      );
    }
                ...
                <MDBTableBody>
                  <tr>
                    {loading ? (
                     ...
                     <tr>
                      <td colSpan={8} className={styles.noDataFound}>No Data Found</td>
                    </tr>
